...
class MainWP_Manage_Sites_List_Table {
	...
	public static function render_page_navigation_left_items() {
		...
		?>
		<div data-tooltip="<?php echo esc_html( 'Switch between Table or Grid view, or toggle the sites filters element visibility', 'mainwp' ); ?>" data-position="top right" data-inverted="">
					...
					?>
						<?php if ( 'cb' === $column_name ) { ?>
							...
							<?php } elseif ( 'login' === $column_name ) { ?>
								...
								?>
								<a href="<?php echo esc_url( $website['url'] ); ?>" class="mainwp-may-hide-referrer open_site_url" target="_blank"><?php echo MainWP_Utility::get_nice_url( $website['url'] ); ?></a>
								<?php
	...
	protected function single_row_columns( $website, $good_health = false ) { // phpcs:ignore -- complex function. Current complexity is the only way to achieve desired results, pull request solutions appreciated.
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			<?php } elseif ( 'login' === $column_name ) { ?>
				...
				?>
				<td class="mainwp-url-cell collapsing"><a href="<?php echo esc_url( $website['url'] ); ?>" class="mainwp-may-hide-referrer open_site_url" target="_blank"><?php echo MainWP_Utility::get_nice_url( $website['url'] ); ?></a></td>
				<?php
			} elseif ( 'tags' === $column_name ) {
				?>
