...
class Cost_Tracker_Admin {
	 ...
	 */
	public function hook_get_default_cost_fields( $default = null, $field = 'all' ) {
		unset( $default );
	...
	public static function get_default_fields_values( $field = 'all' ) {
		...
		$license_types     = array(
			...
			'multi_site'  => esc_html__( 'Multi-Site License', 'mainwp' ),
		);
	...
	public static function handle_settings_post() {
		if ( ! isset( $_POST['mwp_cost_tracker_settings_submit'] ) || ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( $_POST['nonce'] ), 'module_cost_tracker_settings_nonce' ) ) {
			return;
		}

		$all_opts        = Cost_Tracker_Utility::get_instance()->get_all_options();
		$currency        = sanitize_text_field( wp_unslash( $_POST['mainwp_module_cost_tracker_settings_currency'] ) );
		$currency_format = wp_unslash( $_POST['mainwp_module_cost_tracker_currency_format'] );
		$currency_format = Cost_Tracker_Utility::validate_currency_settings( $currency_format );

		$cust_product_types = isset( $_POST['cost_tracker_custom_product_types'] ) ? wp_unslash( $_POST['cost_tracker_custom_product_types'] ) : array();
		$cust_product_types = self::validate_custom_settings_text_fields( $cust_product_types );

		$cust_payment_methods = isset( $_POST['cost_tracker_custom_payment_methods'] ) ? wp_unslash( $_POST['cost_tracker_custom_payment_methods'] ) : array();
	 ...
	 * @param array $fields fields to valid.
	 *
	...
	public static function mainwp_help_content() {
		$allow_pages = array( 'ManageCostTracker', 'CostTrackerAdd', 'CostTrackerSettings' );
		if ( isset( $_GET['page'] ) && in_array( $_GET['page'], $allow_pages, true ) ) {
			?>
	...
	public static function generate_next_renewal( $subscription ) {
		...
		if ( $day1 > $current_time ) {
			...
			$renewal_html = '<i class="yellow bell icon"></i>' . esc_html( $renewal_html );
