...
PRIMARY KEY  (`id`)  '; }
	...
	public function get_cost_tracker_by( $by = 'id', $value = null, $params = array() ) {
		...
		if ( 'id' === $by && is_numeric( $value ) ) {
			$sql = $wpdb->prepare( 'SELECT * FROM ' . $this->table_name( 'cost_tracker' ) . ' WHERE `id`=%d ', $value );
			...
			if ( in_array( $product_type, array( 'plugin', 'theme' ), true ) ) {
				$sql    = $wpdb->prepare( 'SELECT * FROM ' . $this->table_name( 'cost_tracker' ) . ' WHERE `slug`=%s AND product_type = %s ', $value, $product_type );
				...
				$sql = $wpdb->prepare( 'SELECT * FROM ' . $this->table_name( 'cost_tracker' ) . ' WHERE `slug`=%s AND product_type = %s ' . $where, $value );
			}
		}
	...
	public function delete_cost_tracker( $by = 'id', $value = null ) {
		...
		if ( 'id' === $by ) {
			if ( $wpdb->query( $wpdb->prepare( 'DELETE FROM ' . $this->table_name( 'cost_tracker' ) . ' WHERE id=%d ', $value ) ) ) {
				return true;
			}
		} elseif ( $wpdb->query( $wpdb->prepare( 'DELETE FROM ' . $this->table_name( 'cost_tracker' ) . ' WHERE site_id=%d ', $value ) ) ) {
				return true;
		}
		return false;
	}
	...
	public function get_cost_tracker_info_of_clients( $client_ids, $params = array() ) {
		...
		$sql               = 'SELECT * FROM ' . $this->table_name( 'cost_tracker' );
		$result            = $wpdb->get_results( $sql, OBJECT ); // get all cost trackers.
		if ( $result ) {
			foreach ( $result as $cost ) {
				$sites   = ! empty( $cost->sites ) ? json_decode( $cost->sites, true ) : array();
					...
					foreach ( $clients_site_ids as $client_id => $client_site_ids ) {
						foreach ( $sites as $siteid ) {
							if ( in_array( $siteid, $client_site_ids ) ) {
								...
								if ( ! in_array( $cost->id, $clients_costs_ids ) ) {
			...
			foreach ( $result as $cost ) {
				...
				if ( is_array( $cost_sites ) ) {
					foreach ( $sites_ids as $site_id ) {
						foreach ( $cost_sites as $cost_site ) {
							if ( in_array( $cost_site->id, $client_site_ids ) ) {
								if ( ! isset( $sites_costs[ $client_id ] ) ) {
									$sites_costs[ $site_id ] = array();
								}
