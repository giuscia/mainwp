...
class Cost_Tracker_Utility {
	 ...
	 * @return string
	 */
	public static function cost_tracker_format_price( $price ) {
		...
		return '<span class="cost-tracker-currency-amount"><bdi>' . $formatted_price . '</bdi></span>';
	}
	...
	public static function get_separated_costs_price( $group_prices ) {
		...
		if ( is_array( $group_prices ) ) {
			foreach ( $grouped as $idx => $char ) {
				if ( ! empty( $group_prices[ $idx ] ) ) {
					$formated_prices[ $idx ] = self::cost_tracker_format_price( $group_prices[ $idx ] ) . '/' . $char;
				}
			}
		}
		...
		return self::cost_tracker_format_price( 0 );
	}
