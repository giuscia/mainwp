...
class Cost_Tracker_Dashboard {
	 ...
	 */
	public function render_costs_summary( $subscriptions ) {
		$lifetime_costs = 0;
		$monthly_costs  = 0;
		$yearly_costs   = 0;
		?>
			...
			<div class="center aligned middle aligned column">
				<div class="ui mini vertical statistic">
					...
					<div class="value loading-lifetime-costs-value">
						<?php echo Cost_Tracker_Utility::cost_tracker_format_price( $lifetime_costs ); ?>				
					</div>
				</div>
			</div>
			<div class="center aligned middle aligned column">
				<div class="ui mini vertical statistic">
					...
					<div class="value loading-monthly-costs-value">
						<?php echo Cost_Tracker_Utility::cost_tracker_format_price( $monthly_costs ); ?>
					</div>
				</div>
			</div>
			<div class="center aligned middle aligned column">
				<div class="ui mini vertical statistic">
					...
					<div class="value loading-yearly-costs-value">
						<?php echo Cost_Tracker_Utility::cost_tracker_format_price( $yearly_costs ); ?>
					</div>
				</div>
			</div>
		</div>
	...
	public function render_dashboard_body( $subscriptions ) {
		...
		jQuery( document ).ready( function () {
			...
			jQuery('.loading-lifetime-costs-value').html('<?php echo Cost_Tracker_Utility::cost_tracker_format_price( $lifetime_costs ); //phpcs:ignore -- escaped.  ?>');
			jQuery('.loading-monthly-costs-value').html('<?php echo Cost_Tracker_Utility::cost_tracker_format_price( $monthly_costs ); //phpcs:ignore -- escaped. ?>');
			jQuery('.loading-yearly-costs-value').html('<?php echo Cost_Tracker_Utility::cost_tracker_format_price( $yearly_costs ); //phpcs:ignore -- escaped. ?>');
		} );
		</script>
		<?php
	}
	...
	public function get_dashboard_table_row( $subscriptions, &$output = array() ) {
		...
		$license_types = array(
			'single_site' => esc_html__( 'Single-Site', 'mainwp' ),
			'multi_site'  => esc_html__( 'Multi-Site', 'mainwp' ),
		);
		...
		foreach ( $subscriptions as $subscription ) {
			...
			<tr class="" item-id="<?php echo esc_html( $subscription->id ); ?>">
				...
				<td><?php echo $sub_type_icon; ?> <?php echo 'lifetime' === $subscription->type ? 'Lifetime' : esc_html( ucfirst( $subscription->renewal_type ) ); ?></td>
				...
				<td class="right aligned"><span class="ui large text"><?php echo Cost_Tracker_Utility::cost_tracker_format_price( $subscription->price ); ?></span></td>
				<td><?php echo $last_renewal ? MainWP_Utility::format_date( MainWP_Utility::get_timestamp( $last_renewal ) ) : ''; ?></td>
				...
				<?php if ( empty( $subscription->note ) ) : ?>
					...
					<?php else : ?>
						<a href="javascript:void(0)" class="mainwp-edit-sub-note" data-tooltip="<?php echo substr( wp_unslash( $strip_note ), 0, 100 ); ?>" data-position="left center" data-inverted=""><i class="sticky green note icon"></i></a>
					<?php endif; ?>
						<span style="display: none" id="sub-notes-<?php echo intval( $subscription->id ); ?>-note"><?php echo wp_unslash( $esc_note ); ?></span>
				</td>
		...
		<div class="ui modal" id="mainwp-module-cost-tracker-sites-screen-options-modal">
			...
			<div class="scrolling content ui form">
				...
				<?php wp_nonce_field( 'mainwp-admin-nonce' ); ?>
					<input type="hidden" name="wp_nonce" value="<?php echo wp_create_nonce( 'CostTrackerSitesScrOptions' ); ?>" />
